...
func (MM1 MealyMachine) Equal(MM2 MealyMachine) {
	if MM1.n != MM2.n || MM1.m != MM2.m || MM1.q0 != MM2.q0 {
		fmt.Printf("NOT EQUAL")
		return
	}
	for i := 0; i < MM1.n; i++ {
		for j := 0; j < MM1.m; j++ {
			if MM1.δ[i][j] != MM2.δ[i][j] || MM1.ϕ[i][j] != MM2.ϕ[i][j] {
				fmt.Printf("NOT EQUAL")
				return
			}
		}
...
func main() {
	InputMM().AufenkampHohn().Canon().Equal(*InputMM().AufenkampHohn().Canon())
}
